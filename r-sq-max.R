...
filled.contour(ix,iy, matrix(g$r2max, nrow = length(x), ncol=length(y), byrow=TRUE), levels=lv, col=cm.colors(length(lv)),
               ...
               ylab="% MAF(b)",
               plot.axes = {
                 abline(h=log(matrix(seq(0.0001,0.001,0.0001) %*% t(c(1,10,100,1000)))[1:35],10), col = "gray", lty = "dotted")
                 abline(v=log(matrix(seq(0.0001,0.001,0.0001) %*% t(c(1,10,100,1000)))[1:35],10), col = "gray", lty = "dotted")
                 ...
                 axis(1, at=log(labels,10), labels = labels*100)
                 axis(2, at=log(labels,10), labels = labels*100, las=1)
